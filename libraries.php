...
class processSavedOptions
{
    ...
    {
        if(isset($this->savedOptions['create_table']) && isset($options['h'])  && isset($options['p']))
        {
            /**
...
//        $table = $this->tablename;
    ...
    public function checkInput($name, $surname, $email)
    {
        ...
        ctype_upper()
        ctype_lowe()*/
        $this->checkConvert($name, $surname);
...
class ProcessForDatabase
{


    public function checkBeforeProcessDB()
    {
        /**
         *  IF 'file(insert)' 'dryRun(insert but not commit) or 'create_table' check if User DB and table exist
         *  select from DB - check if user exist
         *  Insert -
         *  * check data
         *  DUP(system) - table contraint
         */

        //  createUser createDB createTable
        // SELECT if user exist - if not error
        //
        // SELECT if user DB exist
        //
        // SELECT if user TABLE exist
        return true;
